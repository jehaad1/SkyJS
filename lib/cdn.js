  ...
  function updateState(value) {
    ...
    Merges.forEach((Merge, MergeId) => {
      ...
      if (Merge[0].nodeType === 2)
        return (Merge[0].value = Merge[3].replaceAll(
          MergeId,
          Merge[2](
            Merge[1].map((state) =>
              state === stateId ? stateValue : States.get(state)
            )
          )
        ));
...
function Document(htmlString) {
  ...
  return [...doc.documentElement.querySelector("body").children].length === 1
    ? doc.documentElement.querySelector("body").children[0]
    : "";
}
...
function allChildren(element) {
  ...
  for (let i = 0; i < element.children.length; i++) {
    if (element.children[i].children)
      elements = [
        ...elements,
        ...allChildren({ children: element.children[i].children }),
      ];
  }
  return elements;
}
...
function Render(elements) {
  for (let element of elements) {
    for (let node of element.childNodes) {
      if (
        node.nodeType === 3 &&
        node.textContent.match(/\(\{\(\{Sky(List)?State\-\d+\}\)\}\)/g)
      ) {
        Elements.add([
          node,
          node.textContent.replaceAll("({({SkyListState-", "({({SkyState-"),
        ]);
        if (node.textContent.match(/\(\{\(\{Sky(List)?State\-\d+\}\)\}\)/g))
          node.textContent
            .match(/\(\{\(\{Sky(List)?State\-\d+\}\)\}\)/g)
            .forEach((match) => {
      ...
      if (node.textContent.match(/\(\{\(\{SkyMerge\-\d+\}\)\}\)/g)) {
        if (
          node.textContent.match(/\(\{\(\{SkyMerge\-\d+\}\)\}\)/g).length > 1
        ) {
          node.textContent = node.textContent.replaceAll(
            /\(\{\(\{SkyMerge\-\d+\}\)\}\)/g,
            ""
          ...
          if (!Merges.has(match) || typeof Merges.get(match)[1] !== "function")
            return;
